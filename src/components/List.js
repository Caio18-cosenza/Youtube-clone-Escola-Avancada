    ...
    return (
        <FlatList data={DATA} renderItem={({ index, item }) => (
            <ViewStyled className="mt-5">
                <ImageStyled source={{ uri: item.thumbnail }} defaultSource={{ uri: item.thumbnail }} className={'w-full h-52'} />
                <TextStyled className="text-white font-bold pl-1 mt-1"> {item.title} </TextStyled>
        ...
        )} keyExtractor={({ id }) => id} style={{ marginBottom: 5 }} />
    )
}